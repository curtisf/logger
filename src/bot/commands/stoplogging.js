...
module.exports = {
  func: async (message, suffix) => {
    ...
    if (events.length === 0 && suffix) {
      message.channel.createMessage(`<@${message.author.id}>, none of the provided events are valid to be unset. Look at ${global.envInfo.GLOBAL_BOT_PREFIX}help to see what is valid.`)
  ...
  examples: `\`${global.envInfo.GLOBAL_BOT_PREFIX}stoplogging\` <- stops logging every event configured to log to the channel it's used in
