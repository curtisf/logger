...
module.exports = {
  ...
  \`${global.envInfo.GLOBAL_BOT_PREFIX}setchannel guildMemberAdd, guildMemberRemove, guildMemberKick\` <- joins, leaves, kicks logging **(YOU MUST ALLOW LOGGER __MANAGE CHANNELS AND MANAGE SERVER__ FOR JOIN LOGGING TO WORK! Why? Discord does not send invite info without it!)**
  \`${global.envInfo.GLOBAL_BOT_PREFIX}setchannel anyevent\` <- set events one-by-one to log. Use commas for multiple. Valid events:
