module.exports = {
  func: async (message, suffix) => {
    ...
    if (suffix) {
      if (!global.bot.commands[suffix] || global.bot.commands[suffix]?.hidden) {
        return message.channel.createMessage(`<@${message.author.id}>, that isn't a valid command. Use \`${global.envInfo.GLOBAL_BOT_PREFIX}help\` to see all commands.`)
      }
      ...
      const embed = {
        description: `Below, you can see my commands listed by name and description. To learn more about a command or view examples, use ${global.envInfo.GLOBAL_BOT_PREFIX}help commandname.`,
        color: 3553599,
        timestamp: new Date(),
        footer: {
  ...
  examples: `\`${global.envInfo.GLOBAL_BOT_PREFIX}help\` <- DM a help message with every command
  \`${global.envInfo.GLOBAL_BOT_PREFIX}help setchannel\` <- get further info (examples) on any command`,
  type: 'any',
  category: 'General'
}
