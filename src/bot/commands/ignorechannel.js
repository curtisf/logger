const ignoreChannel = require('../../db/interfaces/sqlite').ignoreChannel
...
module.exports = {
  func: async (message, suffix) => {
    ...
    if (suffix && !isNaN(parseInt(suffix))) {
      ...
      if (!channelToIgnore || !(channelToIgnore.type === 2 || channelToIgnore.type === 0)) return message.channel.createMessage(`Usage: ${global.envInfo.GLOBAL_BOT_PREFIX}ignorechannel OR ${global.envInfo.GLOBAL_BOT_PREFIX}ignorechannel channelID`)
      toIgnore = suffix
    }
  ...
  examples: `\`${global.envInfo.GLOBAL_BOT_PREFIX}ignorechannel\` <- ignore events from the channel this is ran in
  \`${global.envInfo.GLOBAL_BOT_PREFIX}ignorechannel voice channel id\` <- ignore voice events related to the given channel
  \`${global.envInfo.GLOBAL_BOT_PREFIX}ignorechannel text channel id\` <- ignore text-related events related to the given channel`,
  type: 'custom',
  perm: 'manageChannels',
  category: 'Logging'
}
