  ...
  handle: async (guild, member) => {
    ...
    if (log && Date.now() - ((log.id / 4194304) + 1420070400000) < 3000) {
      ...
      event.embed.fields.push({
        name: 'Created At',
        value: `${global.bot.guildSettingsCache[guild.id].makeFormattedTime(member.createdAt)} (${Math.abs(((new Date().getTime() - member.createdAt) / 1000 / 60 / 60 / 24)).toFixed(0)} days, ${((new Date().getTime() - member.createdAt) / 1000 / 60 / 60).toFixed(0)} hours old)`
      }, {
        name: 'Reason',
      ...
      event.embed = {
        ...
        color: 16711680,
        description: `${member.username}#${member.discriminator} left ${guild.features.includes('PREVIEW_ENABLED') ? '(possibly a lurker)' : ''}`,
        fields: [{
          name: 'User Information',
          value: `${member.username}#${member.discriminator} (${member.id}) ${member.mention} ${member.bot ? '\nIs a bot' : ''}`
        }]
