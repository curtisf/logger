...
module.exports = async guildID => {
  ...
  if (idsToCache.length !== 0) {
    ...
    idsToCache.forEach(async channelID => {
      ...
      if (channel && channel.permissionsOf(global.bot.user.id).json['manageWebhooks']) { // Make sure the channel provided in the doc still exists.
        statAggregator.incrementMisc('createWebhook')        
        global.logger.info(`Create hook > C: ${channel.id} G: ${channel.guild.id} `)
