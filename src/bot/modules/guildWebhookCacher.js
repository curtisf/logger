...
module.exports = async (guildID, channelID) => {
  ...
  try {
    ...
    await webhookCache.deleteWebhook(channelID)
  ...
  console.info(`Create hook > C: ${channelID} G: ${guildID} `)
  if (!logChannel) {
    await webhookCache.deleteWebhook(channelID)
    console.log(global.bot.guilds.get(guildID).channels.get(channelID))
    global.bot.guildSettingsCache[guildID].clearEventByID(channelID)
    await clearEventByID(guildID, channelID)
    await cacheGuild(guildID)
  } else {
    const newHook = await logChannel.createWebhook({
      ...
      global.signale.warn(`Leaving guild ${guildID} (${global.bot.guilds.get(guildID).name}, ${global.bot.guilds.get(guildID).memberCount}) because of an error: ${e.message}`)
    })
