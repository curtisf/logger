...
module.exports = async (guildID) => {
  ...
  idsToCache.forEach((id) => {
    if (temp.indexOf(id) < 0) temp.push(id) // A unique value algo
  })
  idsToCache = temp
  let webhooks = await global.bot.guilds.get(guildID).getWebhooks()
  ...
  if (idsToCache.length !== 0) {
    ...
    let guild = global.bot.guilds.get(guildID)
    idsToCache.forEach(async (channelID) => {
      let channel = guild.channels.get(channelID)
      await global.redis.del(`webhook-${channelID}`)
      if (channel) { // Make sure the channel provided in the doc still exists.
        let newHook = await channel.createWebhook({
          name: 'Logger',
        ...
        // Remove channel from guild settings event log cache, and from the guild doc.
